...
function writeAppConfig(){
  ...
  fse.writeJsonSync(path.join(appConfigPath, '_build-generate_config.json'), appConfig);
}
...
function mergeAndWriteWidgetManifests(){
  ...
  fse.writeJsonSync(path.join(basePath, 'widgets/_build-generate_widgets-manifest.json'), resultJson);
}
