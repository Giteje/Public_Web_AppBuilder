...
function useMiddleWares() {
  ...
  app.use(function(req, res, next) {
    ...
    signinInfoRest.getCurentInfoApi(function(err, info) {
      var redirectUrl;
      if (err || info === null) {
      ...
      var token = utils.getTokenFromRequest(info.portalUrl, req);
      if (!token) {
  ...
  //set portal url cookie for default webapp
  app.use(function(req, res, next) {
    if (/webapp$/.test(req.path) ||
      /webapp\/$/.test(req.path) ||
      /webapp\/index.html$/.test(req.path) ||
      /webappbuilder\/stemapp$/.test(req.path) ||
      /webappbuilder\/stemapp\/$/.test(req.path) ||
      /webappbuilder\/stemapp\/index.html$/.test(req.path)) {
      if(req.cookies && req.cookies.wab_portalurl){
        next();
        return;
      }
      signinInfoRest.getCurentInfoApi(function(err, info){
        ...
        res.cookie('wab_portalurl', info.portalUrl);
        next();
      });
    }else{
      next();
...
function mapUrl() {
  ...
  app.get('/builder/rest/apps/list/:username', appRest.getAppList);
  ...
  app.post('/builder/rest/apps/upload', multipart({
    maxFilesSize: '50mb',
  ...
  /************** setting rest ***********************/
  app.get('/builder/rest/signininfo/getcurrentinfo', signinInfoRest.getCurentInfo);
  app.get('/builder/rest/signininfo/getappid', signinInfoRest.getAppId);
